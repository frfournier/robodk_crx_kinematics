...
constexpr int kRoboDkSolutionStride = 2 * crx::kDofCount;

auto SolveFkApi(const real_T *joints, real_T pose[crx::kPoseElementCount],
...
auto SolveIK(const real_T pose[16], real_T *joints, real_T *joints_all,
  ...
  if (joints_approx != nullptr) {
    if (!crx::RoboDkJointsDegCoupledToUserRad(joints_approx,
                                              approx_joints_rad)) {
      return -1;
    }
